...
public class eskensmaikel_mondriaan_final extends PApplet {
...
public void setup() {  // setup van het programma, alles hierbinnen wordt eenmalig uitgevoerd
  ...
  x_speed = PApplet.parseInt (random(8, 22)); // x_speed heeft als beginwaarde een willekeurig getal tussen 8 en 22
  y_loc = 20; // y_loc heeft als begin waarde 20
  y_speed = PApplet.parseInt (random(8, 22)); // y_speed heeft als beginwaarde een willekeurig getal tussen 8 en 22
...
public void draw() {  // void draw voert alles hierbinnen een bepaald aantal keer per seconde uit aan de hand van de frameRate
  if (timer<7000) {  // if statement, als de timer kleiner is dan 7000 wordt alles hierbinnen uitgevoerd
    ...
    grootte=(PApplet.parseInt(random(5, 17))); // variabele grootte heeft een willekeurig getal tussen de 5 en 17, zonder decimalen)
    ...
    if (x_loc>width-20) { // als de x_loc groter is dan de totale breedte van het scherm min 20, dan...
      x_speed = PApplet.parseInt (random(8, 18));  // willekeurig getal voor de x_speed tussen 8 en 18. De x_speed krijgt een nieuwe snelheid telkens als het vierkantje de rechterzijde raakt.
    ...
    if (y_loc>height-20) { // als de y_loc groter is dan de totale hoogte van het scherm min 20, dan...
      y_speed = PApplet.parseInt (random(8, 18));  // willekeurig getal voor de y_speed tussen 8 en 18. De y_speed krijgt een nieuwe snelheid telkens als het vierkantje de onderzijde raakt.
    ...
    int c = img.get(x_loc, y_loc);  // variabele 'c' wordt aangemaakt met als functie color. Gebruikt img (de afbeelding van Victory Boogie Woogie) zonder deze te laten zien als referentie materaal en pakt aan de hand van de x en y positie (x_loc en y_loc) een kleur.
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "eskensmaikel_mondriaan_final" });
  }
}
